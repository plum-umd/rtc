...
#      lang_info[k][:lang_methods] = lang_methods
  ...
  def self.get_inter_edges(e, parent, cls, name_method_map, intra_edges)
    edges = []
    e0m = Dsl::MethodID.new(cls, e[0])
    e1m = Dsl::MethodID.new(cls, e[1])
    ...
    e0ms = name_method_map[e0m].select {|x| 
      ...
      Dsl::MethodID.new(p.cls, p.name) == parent
    }
...
#  def self.get_lang_orders
  def self.write_test_gen_inputs(filename)
    ...
    sid_method_map.each {|k, x|
      new_key = Dsl::MethodID.new(x.cls, x.name)
    ...
    lang_order_map.each {|cls, lang_orders|
      ...
      lang_orders.each {|o|
        ...
        entry_method = Dsl::MethodID.new(parent_info.cls, parent_info.name)
        ...
        missing_methods_from_lang.each {|m|
          ...
          ms.each {|x|
            ...
            first_methods.each {|y|
              ...
              xp = Dsl::MethodID.new(xpi.cls, xpi.name)
              #xp = Dsl::MethodID.new(x.cls, x.name)
...
class Object
  ...
  def dsl_log(fun, sid, target, *args, &blk)
    Dsl::Structure.call_stack = [] if not Dsl::Structure.call_stack
    Dsl::Structure.orders = {} if not Dsl::Structure.orders
    Dsl::Structure.sid_method_map = {} if not Dsl::Structure.sid_method_map

    fun = fun.to_sym
    p = Dsl::Structure.call_stack[-1]

    if ($top_id == self.object_id) and (p and target == 0)
      c = get_class

      if c.instance_methods.include?(fun)
        m = Dsl::Method.new(c, fun, sid, p)

        Dsl::Structure.sid_method_map[sid] = m

        if p
          if not Dsl::Structure.orders.keys.include?(p)
            Dsl::Structure.orders[p] = []
          end
          
          Dsl::Structure.orders[p].push(sid)
        end

        Dsl::Structure.call_stack.push(sid)
        r = self.send(fun, *args, &blk)
        Dsl::Structure.call_stack.pop
      end

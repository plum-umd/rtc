...
module Rtc
  ...
  class Proxy
    ...
    def check_args(args)
      ...
      i = 0
      while i < parameter_layout[:required][0]
        if passed_arguments[i].rtc_type != Rtc::Proxy
          return false unless passed_arguments[i].rtc_type.le_poly(method_type.arg_types[i], pl)
        else
          return false unless method_type.arg_types[i].instance_of?(Rtc::Types::ProceduralType)
        end
    ...
    def check_args(passed_arguments, method_type)
      ...
      while i < parameter_layout[:required][0]
        ...
        if passed_arguments[i].rtc_type != Rtc::Proxy
          ...
          b = false unless method_type.arg_types[i].instance_of?(Rtc::Types::ProceduralType)
        end
